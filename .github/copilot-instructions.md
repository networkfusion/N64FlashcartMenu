...
### Header and Platform Guardrails
- If code uses limit macros such as `LONG_MAX`, `INT_MAX`, or `INT_MIN`, include `<limits.h>` in that translation unit.
- Do not rely on transitive includes for standard macros/functions; add the direct standard header required by the symbol used.
- For N64 runtime code under `src/`, prefer libdragon/POSIX-compatible headers and behavior over host-OS conditionals unless explicitly requested.
- Keep host-only compatibility workarounds scoped to test harnesses (`tests/`) unless the user explicitly asks to broaden runtime portability.
